...
## Implementation note (task 5.1–5.3)

Проверено в devtools на работающем приложении: у контролов zoom/геолокации есть wrapper — `button.ymaps3--button.ymaps3--control-button` (40×40px, radius 8px, непрозрачный фон по умолчанию), инжектируемый рантаймом `ymaps3`; внутри него лежат 24px-иконки `.ymaps3--zoom-control__in/__out` и `.ymaps3--geolocation-control`, перекрашиваемые mask-image. Именно к этому селектору добавлены декларации `.glass-over-map` (фон + `backdrop-filter` + border + shadow) в `index.css` (с теми же объявлениями в блоке `prefers-reduced-transparency`); маска акцентной иконки поверх не тронута.
